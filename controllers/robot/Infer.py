...
# Print each CPD in detail before checking model
print("\nDetailed CPDs:")
print("Obstacle CPD:")
print(cpt_O)
print("\nVisible CPD:")
...
def bayesian(dist: float, angle: float) -> tuple[str, float]:
    ...
    action_dist = inference.query(
        variables=['Action'], virtual_evidence=virtual_evidence)
    action_idx = int(np.argmax(action_dist.values))
    ...
    p_success = prob_success_dist.values[0]
