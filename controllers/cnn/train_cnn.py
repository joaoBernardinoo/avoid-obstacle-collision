...
if sys.platform == 'linux':
    DATASET_PATH = Path('/home/dino/SSD/cnn_dataset')
else:
...
def load_dataset(path: Path, max_samples=None):
    """
    Carrega o dataset de arquivos .npz.
    """
    cam_images, lidar_data, targets = [], [], []

    if not path.exists():
        print(f"Error: Dataset path not found at '{path}'")
    ...
    X_cam = np.array(cam_images, dtype=np.float32) / 255.0
    X_lidar = np.array(lidar_data, dtype=np.float32)
    y = np.array(targets, dtype=np.float32)

    # --- NORMALIZE TARGETS (y) ---
    # Normalize distance (column 0) to the range [-1, 1]
    # The expected maximum absolute value is 3.14.
    y[:, 0] /= 3.14
    # Normalize angle (column 1) to the range [-1, 1]
    # The expected maximum absolute value is pi.
    y[:, 1] /= np.pi
    # Clip values to ensure they are strictly within [-1, 1]
    y = np.clip(y, -1.0, 1.0)
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_cam)):
        ...
        print(
            f"\nFold {fold+1} training complete. Best model is saved at '{checkpoint_path}'")
    ...
    if fold_histories:
        avg_history = {
            ...
            'val_mae': np.mean([h['valcheckpoint_mae'] for h in fold_histories], axis=0).tolist()
        }
